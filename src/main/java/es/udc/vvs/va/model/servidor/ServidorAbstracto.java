...
/**
 * Representa la abstraccion de un servidor, cuyo comportamiento real es comun a
 * muchos tipos de servidor,
 * 
 */
public abstract class ServidorAbstracto implements Servidor {
  ...
  /**
   * La estructura tokens para gestionar las operaciones basicas como add(),
   * remove() o contains()... eficienemente (log(n)). Son los tokens validos del
   * servidor.
   * 
   ...
   * Para mas informacion
   * 
   * @see es.udc.vvs.vas.Servidor#eliminar( es.udc.vvs.vas.Contenido,
   * java.lang.String)
   */
  @Override
   ...
   * 
   * @param contenido
   *  contenidos a los que agregar anuncios
  ...
  public Collection<Contenido> buscar(String subcadena, String token)
    ...
    for (Contenido c : almacen) {
      if (c.obtenerTitulo().toLowerCase().contains(subcadena.toLowerCase())) {
        busqueda.add(c);
      }
    }
  ...
  /**
   * Este metodo genera nuevos tokens arbitrarios cada vez que se llama. Puede
   * que quieras extraer este comportamiento a algun otro lugar y/o hacerlo mas
   * seguro.
   * 
  ...
  /**
   * Devuelve el tokenMagico. Usa este metodo cuidadosamente. Definido para la
   * validacion automatica por si se genera un token arbitrario y se quiere
   * conocer.
   * 
