...
public class ContenidoImpTest extends ExecutionContext implements ContenidoTest {
  public static final Path MODEL_PATH = Paths
      .get("es/udc/vvs/va/test/model/testautomation/ContenidoTest.graphml");
  ...
  public void e_eliminar_contenido() {
    ...
    System.out
        .println("Borrando contenido de listResult cuando tiene >1 contenido");
  ...
  public void runSmokeTest() {
    // xeramos  un test que chegue a este estado
    new TestBuilder()
        .setModel(MODEL_PATH)
        .setContext(new ContenidoImpTest())
        .setPathGenerator(
            new AStarPath(new ReachedVertex("v_Emisora_Con_Contenido")))
  ...
  public void runFunctionalTest() {
    new TestBuilder().setModel(MODEL_PATH).setContext(new ContenidoImpTest())
        .setPathGenerator(new RandomPath(new EdgeCoverage(100))) // xera tantos
                                                                 // test como
                                                                 // sexan
                                                                 // necesarios
                                                                 // para pasar
  ...
  public void runStabilityTest() {
    new TestBuilder()
        .setModel(MODEL_PATH)
        .setContext(new ContenidoImpTest())
        .setPathGenerator(new RandomPath(new TimeDuration(5, TimeUnit.SECONDS))) // atravesamos
                                                                                 // aleatoriamente
                                                                                 // o
                                                                                 // grafo
                                                                                 // durante
